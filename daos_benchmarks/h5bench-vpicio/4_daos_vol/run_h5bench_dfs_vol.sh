...
# The DAOS VOL aims for 1MiB chunks by default, but I think for performance testing we aimed for around 16-32 MiB chunks, which may be worth trying. 
# Try again with the latest PR at VOL which has DAOS_OCH_SHD_MAX for datasets by default, if that improves the performance compared to MPIIO backends. 
